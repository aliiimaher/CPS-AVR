...
int prev_toggle = 1;
// todo: check the receivedData type.
char receivedData;
uint32_t receivedDataTest;
//
...
// lightening
double lightningMonitoringDutyCycle = 20; 
float ldrValue;
// ===================================================
...
int main(void) {
    ...
    while (1) {
        ...
        while (((SPSR >> SPIF) & 1) == 0);
        // receivedData = SPDR;
        receivedDataTest = SPDR;

        LCD_write('t');
        // ==================== Part3 ====================
        // receiving data
        if (receivedDataTest >= 150) {
            numberOfInputs--;
            ldrValue = (receivedDataTest) - 150;
            // char buffer[3];
            // itoa((int)(ldrValue), buffer, 10);
            // for (int i = 0; i < 3; i++) LCD_write(buffer[i]);
            LCD_write('p');
        }
...
void LDRTimerCounterConfig() {
    ...
    DDRD |= (1 << PORTD7);
    TCCR2 = 0x00;
    TCCR2 |= (1 << WGM01) | (1 << WGM00);           // fast pwm
    TCCR2 |= (1 << COM01);                          // clear Oc0 on compare match
    TCCR2 |= (1 << CS01);                           // 1/8 clock prescaling
